...
if [[ $lang == 'fr' ]]
then
	grep -E -o "(\w+\W+){0,5}$motif\b(\W+\w+){0,5}" $fichier_text | sed -r "s/(.*)$motif(.*)/<tr><td class="has-text-right">\1<\/td><td class="has-text-danger">\2<\/td><td class="has-text-left">\3<\/td><\/tr>/"
elif [[ $lang == 'ru' ]]
then
